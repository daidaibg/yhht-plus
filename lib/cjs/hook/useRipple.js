...
const useKeepAnimation = require("./useKeepAnimation.js");
const useConfig = require("./useConfig.js");
const setStyle = require("../utils/set-style.js");
const period = 200;
...
function useRipple(el, fixedRippleColor) {
  const rippleContainer = vue.ref(null);
  const classPrefix = useConfig.usePrefixClass();
  const { keepRipple } = useKeepAnimation.default();
  const handleAddRipple = (e) => {
    ...
    if (rippleContainer.value.parentNode === null) {
      setStyle.default(rippleContainer.value, {
        position: "absolute",
        left: `${0 - border}px`,
        top: `${0 - border}px`,
        width: `${width}px`,
